...
- **v1.5**: Added `is_locked` mechanism on `metric_threshold` rows to support compliance use cases (regulated tenants pinning a metric at a given scope so narrower scopes cannot override). `is_locked` is a `p2` FR on the write side and a resolution-logic rule on the read side. Resolution walks least-specific to most-specific and stops at the first locked row; a tenant admin locking the `tenant` row blocks team/role/team+role overrides for that metric in that tenant. Write CRUD rejects narrower-scope writes that would be shadowed by a locked row with a structured `403 threshold_locked` error identifying the blocking scope. `is_locked` is **not** exposed in the v1 `POST /catalog/get_metrics` response — Dashboard Configurator does not need it for rendering, and admin UI is out of v1 scope. It becomes an additive field when admin UI ships. Governance questions (who can set locks, audit fields, lock expiry, product-default locks) tracked as Open Questions.
...
**Role**: Serves the catalog via `POST /catalog/get_metrics` with resolved per-tenant thresholds. Exposes admin CRUD for thresholds. Validates metadata integrity at the API layer.

#### Catalog Consumer

**ID**: `cpt-metric-cat-actor-consumer`

**Role**: Any backend or frontend component that reads `POST /catalog/get_metrics` and relies on the returned metadata. In v1 this is Dashboard Configurator; in later waves, it will include alerting, reports, scheduled digests, and the admin UI.
...
- `POST /catalog/get_metrics` read endpoint returning catalog + resolved thresholds for a request's `(tenant, role, team)` context
...
- Cache layer (configurable TTL, default 5 minutes) fronting `POST /catalog/get_metrics`
...
The system **MUST** front the read endpoint with a **server-side** cache layer whose default TTL is 5 minutes and whose cache key includes at minimum the tenant identifier (plus `role_slug` / `team_id` when supplied in the body). Admin writes **MUST** invalidate the cache for the affected tenant so threshold changes appear on the next page load. **HTTP-level and CDN-level caching are not in scope** — `POST /catalog/get_metrics` is not cacheable by HTTP intermediaries, and the read endpoint deliberately uses POST to keep `role_slug` / `team_id` out of access logs (`cpt-metric-cat-fr-read-endpoint`). The remaining cache-implementation choice (shared Redis vs pub-sub-coordinated in-process) is a DESIGN decision tracked in §13 OQ and bounded by `cpt-metric-cat-nfr-cross-replica-invalidation`; this FR mandates that a cache exists and that its invalidation correctly tracks admin writes.
...
The system **MUST** seed the initial catalog by importing the metadata currently hardcoded in `src/screensets/insight/api/thresholdConfig.ts` (`BULLET_DEFS`, `IC_KPI_DEFS`) and `src/screensets/insight/types/index.ts` (`METRIC_KEYS`). The seed is a one-time export that the product team reviews before merging. After the seed lands and the frontend consumes `POST /catalog/get_metrics`, the duplicated metadata on the frontend **MUST** be removed. The rollout **SHOULD** go through a short transitional phase where the frontend fetches the catalog but retains local metadata as a fallback; the removal of the fallback ships in the immediately following release. Byte-for-byte comparison between rendered output before and after migration is a rollout gate.
...
`POST /catalog/get_metrics` p95 response time **MUST** be ≤ 100ms on a cache hit and ≤ 500ms on a cache miss for a tenant with up to 200 catalog rows.
...
A successful threshold write **MUST** be visible to subsequent `POST /catalog/get_metrics` calls from the same tenant within one cache TTL, with no manual invalidation required.
...
**Protocol/Format**: Consumers **MUST** fetch `POST /catalog/get_metrics` once per session (or per cache-TTL window), key metadata lookups by `metric_key`, and degrade gracefully when a `metric_key` is absent from the response. Consumers **MUST NOT** hardcode metric metadata that the catalog provides.
...
5. Next `POST /catalog/get_metrics` for the tenant returns the new threshold
...
4. `POST /catalog/get_metrics` returns the new entry after cache expiry or invalidation
...
1. Consumer calls `POST /catalog/get_metrics`
...
- [ ] Admin write on replica A makes the change visible to `POST /catalog/get_metrics` served by replica B within `cpt-metric-cat-nfr-cross-replica-invalidation`'s threshold
...
- [ ] `POST /catalog/get_metrics` accepts optional `role_slug` and `team_id` fields in the JSON request body and resolves thresholds per the precedence `team+role → team → role → tenant → product-default`
- [ ] `POST /catalog/get_metrics` surfaces `resolved_from` per metric threshold (value in `{ team+role, team, role, tenant, product-default }`) so consumers can explain which scope supplied each value
...
- [ ] `POST /v1/admin/metric-thresholds` persists a new threshold at any v1 scope and invalidates the tenant's cache; the value is visible to the next `POST /catalog/get_metrics` for the matching `(tenant, role, team)` context without waiting for TTL
...
| Threshold override precedence misunderstood by consumers | Inconsistent bullet colors across surfaces for the same metric | `cpt-metric-cat-fr-scoped-thresholds` fixes precedence; the `POST /catalog/get_metrics` response always returns `resolved_from` so the winning scope is visible without a diagnostics call |
...
| Team-scoped threshold written by a team lead contradicts a tenant admin's rollout intent | Org-wide comparability fragments silently across teams | The `POST /catalog/get_metrics` response surfaces `resolved_from`, so tenant admins can audit which teams have overrides and why; governance on team-scoped thresholds (justification, expiry, caps) is tracked as an OQ and revisited once the first team-lead authoring surface lands |
...
| **#223 — gold views honest-null contract** (merged) | Unblocks the "no `null%` / `nullh` in rendered DOM" story for consumers of `POST /catalog/get_metrics`. |
...
3. Analytics-api: add `POST /catalog/get_metrics` (with server-side cache, optional `role_slug` / `team_id` body fields, and `resolved_from`) plus `POST/PUT/DELETE /v1/admin/metric-thresholds` accepting all v1 scopes.
4. Frontend: hydrate from `POST /catalog/get_metrics` alongside existing local metadata as fallback; ship a release that introduces this dual-path and passes the current viewer's `(role, team)` in the request body where known.
